...
namespace JInput {
	...
	public class Controller {
        ...
        public float getTriggerValue(bool a_Left) {
            //xbox controllers have their trigger axes set to one axis, this will use unitys when it's a xbox controller
            if (a_Left) {
                if (isXboxController) {
                    return Input.GetAxisRaw("LeftFire");
                }else {
                    return (getAxisValue(ControllerAxes.LeftTrigger) + 1.0f) / 2.0f;
                }
            }else {
                if (isXboxController) {
                    return Input.GetAxisRaw("RightFire");
                } else {
                    return (getAxisValue(ControllerAxes.RightTrigger) + 1.0f) / 2.0f;
                }
            }
        }
		...
		internal void updateController() {
            ...
            Vector2 leftStick = getAxisValue(ControllerVec2Axes.LStick);
            //Debug.Log(leftStick.magnitude);
            if (Mathf.Abs(leftStick.magnitude) <= deadZone) {
                m_Data.axesValues[(int)ControllerAxes.LStickX] = 0;
                m_Data.axesValues[(int)ControllerAxes.LStickY] = 0;
            }
            Vector2 rightStick = getAxisValue(ControllerVec2Axes.RStick);
            if (Mathf.Abs(rightStick.magnitude) <= deadZone) {
                m_Data.axesValues[(int)ControllerAxes.RStickX] = 0;
                m_Data.axesValues[(int)ControllerAxes.RStickY] = 0;
            }
